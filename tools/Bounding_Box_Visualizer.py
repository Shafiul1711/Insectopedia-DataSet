...
class YOLOViewer(tk.Tk):
    ...
    def load_dataset(self, path: Path):
        ...
        self.update()
        try:
            self.entries, self.class_names = scan_dataset(path)
    ...
    def _populate_list(self):
        ...
        last_folder = None
        self._list_indices = []  # maps listbox row → entry index
        ...
        for i, e in enumerate(self._filtered):
            if e['folder'] != last_folder:
                ...
                self._list_indices.append(None)  # folder header
    ...
    def _highlight_list_item(self, idx):
        # Find listbox row for this filtered index
    ...
    def show_image(self, idx):
        ...
        self.status_var.set(f'{name}  —  {self._orig_img.width}×{self._orig_img.height}  —  {label_status}')
    ...
    def redraw(self):
        ...
        for cls, cx, cy, bw, bh in self._boxes:
            ...
            # Draw box (thick border via multiple rects)
            for t in range(2):
